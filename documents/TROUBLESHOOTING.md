...
# 如果设备地址是 0x3d，修改配置
nano ~/rpi-mediaplayer/ts_app/ts.ini
# 修改: address = 0x3D

# 重启服务
systemctl --user restart ts
```

</details>

<details>
<summary><strong>步骤 3：手动测试 Touchscreen</strong></summary>
...
# 2. 运行测试脚本（显示 20 秒）
python3 /usr/local/bin/ts_display.py
...
# 如果没有，重新安装
pip install --upgrade luma.ts
```

**测试成功标准：**
- ✅ Touchscreen 显示 "Hello Touchscreen!"
- ✅ 显示当前时间并每秒更新
- ✅ 20 秒后自动退出
...
<details>
<summary><strong>步骤 4：检查 Touchscreen 服务日志</strong></summary>

```bash
# 查看服务状态
systemctl --user status ts

# 查看详细日志
journalctl --user -u ts -n 50

# 实时日志
journalctl --user -u ts -f
```
...
| `OSError: cannot open resource` | 字体文件缺失 | `ls ~/rpi-mediaplayer/ts_app/msyh.ttf` |
...
**症状：**
- Touchscreen 显示 "System Ready" 后不再更新
- 或显示错误信息

#### 🔍 排查步骤

<details>
<summary><strong>步骤 1：检查 Touchscreen 服务日志</strong></summary>
...
```bash
# 1. 检查 ts.ini
cat ~/rpi-mediaplayer/ts_app/ts.ini | grep PLAYER_ID
...
```bash
# 1. 检查字体文件
ls -l ~/rpi-mediaplayer/ts_app/msyh.ttf

# 2. 如果不存在，从 resources 复制
sudo cp resources/ts/msyh.ttf ~/rpi-mediaplayer/ts_app/

# 3. 设置权限
sudo chown player:player ~/rpi-mediaplayer/ts_app/msyh.ttf
...
# 重新安装最新版本
pip install --upgrade luma.ts requests pillow
...
| 文件 | 行号 | 功能 |
|------|------|------|
| `resources/ts/query.py` | 20 | 定义 `BT_VOLUME_MAX = 127` |
| `resources/ts/query.py` | 233-256 | 实现 `get_bluetooth_volume_dbus()` |
| `resources/ts/state_handlers.py` | 43-75 | 处理蓝牙音量显示逻辑 |
...
# 查看特定时间段日志
journalctl --user -u ts --since "1 hour ago"
journalctl --user -u ts --since "2024-01-01 10:00:00"
```
...
sudo i2cdetect -y 3  # Touchscreen
...
```bash
# 1. 停止所有服务
systemctl --user stop pipewire squeezelite shairport-sync ts volume
sudo systemctl stop bluetooth bluetooth-a2dp-autopair
...
1. **📸 收集诊断信息：**
   ...
   # 生成诊断报告
   {
     ...
     echo "=== 服务状态 ==="
     systemctl --user status pipewire squeezelite shairport-sync ts
