...
**💥 第六大坑：安静环境下的“灵异自唤醒”（误唤醒）**
- **避坑核心：** 神经网络唤醒引擎（如 MicroWakeWord/OpenWakeWord）在极度安静时容易把底噪“幻听”成唤醒词，导致频繁误触发。必须从 HA 实体配置和 Pipeline 层面解决。
- **血泪教训与四大解法：**
  1. **禁用多余唤醒词**：如果你只用 `ok_nabu`，千万不要让 `hey_luna` 等其他唤醒词同时处于激活状态。多个模型同时监听会让误唤醒概率成倍飙升。务必在 HA 的设备页面（或 `preferences.json` 中）将不用的唤醒词禁用或设为 null。
  2. **调低灵敏度阈值（重点）**：HA 界面中暴露的 `Wake Word Sensitivity` 如果是最高值（1.0），几乎必然疯狂误触发。请务必将主力唤醒词的灵敏度下调至 **0.5 ~ 0.7** 之间进行测试。
  3. **修正“停顿检测” (VAD)**：在 HA 侧的语音助手 Pipeline 配置中，有一项 `Finished speaking detection`（停顿检测）。如果设为 `Relaxed`，系统会对静音过度宽容，极易把风扇声等环境音当成你还没说完的话。请务必改回 `Default` 或 `Aggressive`，一旦听不到清晰人声就立刻切断拾音。
  4. **降噪 (Noise Suppression) 的误区**：LVA 的 `Mic Noise Suppression`（建议开到 Max/4）对净化送给 STT（语音识别）的音频非常有效，能提升识别率。但它主要处理平稳噪音，**不能**直接用来防止唤醒词模型的静音幻觉。防误唤醒的核心依然是调低灵敏度与改写 Pipeline VAD。
---
...
### 问题七：播放音乐后无法再次唤醒（TTS 竞态死锁）

**现象：** 当用户对语音助手下达“继续播放”或“打开播放器”等会立刻让 Squeezelite 输出音频的指令后，系统陷入彻底无法再次唤醒的状态（麦克风死锁），而下达其他指令（如问时间、暂停）则可正常连续唤醒。

**根因（并发抢占死锁）：** 
当 HA 下发媒体播放指令后，Squeezelite 瞬间通过 PipeWire 开始高负载输出音频；同时 HA 生成了一句 TTS（“好的”）让 LVA 播放。
此时 `satellite.py` 中的 `tts_player.play` 试图并发播放，可能抛出异常或在执行完毕尝试恢复音量（`unduck`，需调用底层 DBus）时发生阻塞挂起。导致 TTS 播放完毕的回调函数 `_tts_finished` 无法完整执行，关键状态锁 `self._pipeline_active` 永远停留在 `True`。而在 LVA 逻辑中，只要该锁为 `True`，所有唤醒词均被默默丢弃。

**解决方案（防线加固）：**
在 `satellite.py` 中增加两层并发兜底：
1. **10 秒看门狗定时器**：在下发 TTS 播放指令的同时启动 10 秒倒计时。若发生底层异常导致回调丢失，看门狗会强行复位 `_pipeline_active` 状态锁。
2. **异步化音量恢复**：将容易引发 DBus 超时阻塞的 `self.unduck()` 调用转移至后台守护线程中执行，确保主逻辑顺畅发出 `done` 信号给前端 UI。
