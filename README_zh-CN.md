...
#### 🚀 极致性能优化：压榨 4GB 内存 (Maxing out 4GB RAM)

对于主打零延迟响应的媒体播放器与语音助手项目来说，树莓派 SD 卡那可怜的 I/O 吞吐量往往是性能的致命瓶颈。特别是当 `wyoming` 语音服务频繁生成音频切片，或者 `systemd` 疯狂写入日志时，这种微秒级的阻塞会导致前端动画掉帧、触控粘滞或唤醒词响应迟钝。

既然树莓派 4B 拥有 4GB 内存（对于本项目绰绰有余），我们通过 Ansible 的 `system` 角色进行了以下**硬核的底层 I/O 隔离与内存调优**，以达到“绝对零延迟”的目标：

1. **彻底拔除 Swap**：直接卸载并彻底废弃 `dphys-swapfile`。强迫系统 100% 运行在纯物理内存中，永远不用忍受 SD 卡分页交换带来的可怕延迟峰值。
2. **挂载临时目录为 RAM Disk**：将 `/tmp` 和 `/var/tmp` 挂载为 `tmpfs`。系统中所有的临时生成文件（如 Wyoming 的语音切片、中间缓存）都在纯物理内存中瞬时创建和销毁。
3. **日志全盘内存化 (`log2ram`)**：引入 `log2ram` 接管 `/var/log`。所有的底层服务日志直接在内存中运转，仅在正常关机时进行打包落盘，保护 SD 卡寿命的同时消灭日志 I/O 阻塞。
4. **内核 Sysctl 稳健调优**：
   - `vm.vfs_cache_pressure = 50`：告诉内核不要轻易释放目录和文件的索引缓存，让 Python 加载模块、前端读取图标达到“零寻道时间”。
