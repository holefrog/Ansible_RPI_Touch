...
对于主打零延迟响应的媒体播放器与语音助手项目来说，树莓派 SD 卡那可怜的 I/O 吞吐量往往是性能的致命瓶颈。特别是当 `wyoming` 语音服务频繁生成音频切片时，这种微秒级的阻塞会导致前端动画掉帧、触控粘滞或唤醒词响应迟钝。
...
3. **日志持久化存储**：启用 `systemd-journald` 的磁盘持久化，确保服务日志在异常断电或崩溃后仍可回溯，方便调试。
4. **内核 Sysctl 稳健调优**：
