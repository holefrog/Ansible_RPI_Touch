...
class WakeStreamingSatellite(SatelliteBase):
    """Satellite that waits for local wake word detection before streaming. Modified for stealth VAD listening."""
    ...
    async def event_from_mic(
        ...
        if self.is_streaming:
            # Forward to server
            await self.event_to_server(event)
        elif getattr(self, "waiting_for_vad", False) and self.vad is not None:
            import time
            if (self.vad_timeout_seconds is not None) and (time.monotonic() >= self.vad_timeout_seconds):
                # Timeout without speech
                self.waiting_for_vad = False
                self.vad_timeout_seconds = None
                _LOGGER.info("VAD timeout. Going back to waiting for wake word.")
                await self.trigger_streaming_stop()
                await self._send_wake_detect()
                return
            ...
            self.vad_timeout_seconds = None
            self.is_streaming = True
            _LOGGER.info("Speech detected, now streaming to HA")
            await self._send_run_pipeline()
            await self.trigger_streaming_start()
    ...
    async def event_from_wake(self, event: Event) -> None:
        if self.is_streaming or getattr(self, "waiting_for_vad", False):
            return
