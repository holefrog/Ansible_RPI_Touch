...
* **终极修复 2（魔改官方源码，实现本地 VAD 截流黑科技）**：
  * **背景发现**：原版 `wyoming-satellite` 在开启本地唤醒词后，会**强制禁用** `--vad` 参数。这意味着只要唤醒词触发，卫星会立刻连接 HA 导致受制于 HA 的短促静音超时。
  * **解决方案**：我们在树莓派本地深度魔改了官方的 `satellite.py` 源码（并通过 Ansible 的 `satellite.yml` 实现了持久化覆盖）。
  * **工作流原理**：唤醒后，卫星端不再立刻连接 HA，而是进入**“本地隐身监听模式”**。用户可以安稳地思考（最长受 `--vad-wake-word-timeout 5` 控制的 5 秒钟），直到本地底层 VAD 引擎听到用户真正说出第一个字，卫星才会在一瞬间建立连接，并将带有“前半句话”缓冲区的完整音频流上传。完美绕过了 HA 的 2 秒催促倒计时！
