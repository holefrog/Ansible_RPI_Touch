...
# ── UI 事件通知（UDP → assistant_listener.py on port 10701） ─────────────
import socket as _socket
import json as _json

def _send_ui_event(event: str, **kwargs) -> None:
    """向本机 10701 UDP 端口发送 UI 事件，供 assistant_listener.py 消费。"""
    try:
        payload = _json.dumps({"event": event, **kwargs}).encode("utf-8")
...
class VoiceSatelliteProtocol(APIServer):
    ...
    def handle_voice_event(self, event_type: VoiceAssistantEventType, data: Dict[str, str]) -> None:
        ...
        if event_type == VoiceAssistantEventType.VOICE_ASSISTANT_RUN_START:
            ...
            if event_type == VoiceAssistantEventType.VOICE_ASSISTANT_STT_END:
                _send_ui_event("transcript", text=data.get("text", ""))  # → UI 显示识别文字
    ...
    def wakeup(self, wake_word: Union[MicroWakeWord, OpenWakeWord]) -> None:
        ...
        _send_ui_event("awake")  # → UI 弹出语音蒙版 + 暂停音乐
        self._pipeline_active = True
        self.duck()
        self.state.tts_player.play(
            self.state.wakeup_sound,
