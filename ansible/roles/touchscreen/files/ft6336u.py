...
# TP_INT is NC, BCM 4 and 5 are used for Hardware I2C-3
TP_RST   = 17
...
class ft6336u():
    ...
    def read_touch_data(self):
        ...
        try:
            ...
            self.error_count = 0  # 成功读取则重置错误计数
        except Exception:
            self.error_count += 1
            # 连续 50 次 (约 1 秒) 读取不到 I2C，说明芯片假死，直接硬件强制复位
        ...
        if buf:
            ...
            if points > 0 and points <= 2:
                ...
                try:
                    buf_xy = self.read_bytes(TOUCH_XY_REG, 6 * self.point_count)
                except Exception:
                    buf_xy = None
                ...
                if buf_xy and len(buf_xy) >= 6 * self.point_count:
                    for i in range(self.point_count):
                        self.coordinates[i]["x"] = 319 - (((buf_xy[(i * 6) + 0] & 0x0f) << 8) + buf_xy[(i * 6) + 1])
                        self.coordinates[i]["y"] = ((buf_xy[(i * 6) + 2] & 0x0f) << 8) + buf_xy[(i * 6) + 3]
            else:
                self.point_count = 0
    
    def get_touch_xy(self):
        self.read_touch_data() # Ensure we fetch the latest data in polling mode
