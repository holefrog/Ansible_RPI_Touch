...
3. **Log-to-RAM (`log2ram`)**: Introduced `log2ram` to take over `/var/log`. All underlying service logs operate directly in RAM and are only written to the SD card during a graceful shutdown. This protects the SD card's lifespan while eliminating log-related I/O blocking.
