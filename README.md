...
## 🛠️ 硬件接线指南 (Hardware Wiring Guide)

为了让系统能够完美共存，本项目经过精密的硬件重构，彻底解决了音频板与 SPI 屏幕之间的所有潜在物理冲突。

### ⚠️ 核心防冲突设计说明
1. **背光冲突避让**：SPI 屏幕出厂默认的背光引脚 (LCD_BL) 通常占用 BCM 18，但该引脚已被 WM8960 的 I2S CLK 独占。**必须将屏幕背光引脚单独飞线至 BCM 13 (物理引脚 33)**。
2. **触控总线隔离**：FT6336U 触控芯片使用 GPIO 模拟的 **I2C-3** 独立通道，与音频控制的标准 I2C-1 在物理上完全隔离，杜绝杂音干扰。
3. **中断引脚悬空**：系统采用高效的主循环同步轮询模式，因此**触摸中断引脚 (TP_INT) 无需连接**。

---

### 1. WM8960 音频板接线表 (I2C-1 + I2S)
保持独占标准音频与控制总线，作为核心发声单元。

| 树莓派物理 PIN | 树莓派 BCM 引脚 | 音频板接口 | 详细功能描述说明 |
| :---: | :--- | :--- | :--- |
| **2 / 4** | 5V Power | **5V** | 音频板主供电驱动 |
| **6 / 9** | Ground | **GND** | 音频板主接地引脚 |
| **3** | BCM 2 (SDA1) | **SDA** | 硬件 I2C-1 总线数据线 (音频控制) |
| **5** | BCM 3 (SCL1) | **SCL** | 硬件 I2C-1 总线时钟线 (音频控制) |
| **12** | **BCM 18 (CLK)** | **CLK** | **I2S 位时钟 (Bit Clock) - 音频核心，严禁屏幕抢占** |
| **35** | BCM 19 (LRCK) | **LRCLK** | I2S 帧时钟 / 左右声道选择 (Frame Clock) |
| **38** | BCM 20 (DIN) | **ADC** | I2S 录音数据输入线 (Data Input) |
| **40** | BCM 21 (DOUT) | **DAC** | I2S 放音数据输出线 (Data Output) |

---

### 2. SPI 彩色触摸屏接线表 (SPI0 + PWM1 + 软 I2C-3)
包含 ST7789 画面显示与 FT6336U 触摸控制，已避开所有硬件冲突点。

| 树莓派物理 PIN | 树莓派 BCM 引脚 | 屏幕端接口 | 详细功能描述说明 |
| :---: | :--- | :--- | :--- |
| **1** | 3.3V Power | **VCC / 3.3V**| 屏幕控制及触摸芯片主供电 |
| **39** | Ground | **GND** | 屏幕主接地引脚 |
| **19** | BCM 10 (MOSI) | **MOSI** | 标准硬件 SPI0 主输出从输入像素数据 |
| **21** | BCM 9 (MISO) | **MISO** | 标准硬件 SPI0 主输入从输出数据 |
| **23** | BCM 11 (SCLK) | **SCLK** | 标准硬件 SPI0 时钟同步线 |
| **24** | BCM 8 (CE0) | **CS** | 标准硬件 SPI0 片选使能引脚 |
| **22** | BCM 25 | **DC** | 屏幕数据 / 命令控制切换引脚 |
| **13** | BCM 27 | **RST** | 屏幕硬件复位引脚 |
| **33** | **BCM 13 (PWM1)**| **LCD_BL** | **屏幕背光控制 - 使用硬件 PWM1 通道完美避让音频冲突** |
| **7** | BCM 4 | **Touch SDA** | 模拟 I2C-3 总线数据线 (触控数据读取) |
| **29** | BCM 5 | **Touch SCL** | 模拟 I2C-3 总线时钟线 (触控数据读取) |
| **悬空** | N/A | **TP_INT** | 触摸中断引脚 - **采用同步轮询架构，无需接线** |
...
│   │   └── gui_touch/          # Python 虚拟环境构建, NumPy/Pillow 依赖安装, 部署主循环服务
...
如果你需要调整 UI 的布局、字体样式或优化触控碰撞热区，只需修改 `scripts/` 目录下的核心 Python 脚本。部署完成后，可以使用以下命令在树莓派上实时追踪系统运行状态：
